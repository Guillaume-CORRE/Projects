...
knitr::opts_chunk$set(message = FALSE, warning = FALSE, fig.height = 5.5, 
                      fig.width = 12, sanitize = TRUE, echo = FALSE, cache = TRUE)
```
...
tab_fun(round(prop.table(table(realite, resultat),
                         margin = 1), 3),
...
- `r round(table(realite, resultat)[1]/(table(realite, resultat)[1]+table(realite, resultat)[3]) * 100,3)`\% des personnes ayant une moyenne inférieure à 10 sont biens affectées. Ici, on parle de sensibilité.

- `r round(table(realite, resultat)[4]/(table(realite, resultat)[2] + table(realite, resultat)[4])*100,3)`\% des personnes ayant une moyenne supérieure à 10 sont bien affectées. Ici, on parle de spécificité.
...
tab_fun(table(realite_court, resultat_court),
        title="Matrice de confusion LDA court") %>% add_header_above(c("","Prediction"=2))
...
- La sensibilité est de `r round(table(realite_court, resultat_court)[1]/(table(realite_court, resultat_court)[1] + table(realite_court, resultat_court)[3]) * 100,3)`\% , c'est à dire le taux de personnes ayant une moyenne inférieure à 10  qui sont biens affectées. 

- La spécificité est de `r round(table(realite_court, resultat_court)[4]/(table(realite_court, resultat_court)[2] + table(realite_court, resultat_court)[4])*100,3)`\% , c'est à dire le taux de personnes ayant une moyenne supérieure à 10 qui sont biens affectées.
...
```{r}
ggroc(tab_lda_court) + ggtitle("Courbe roc lda_court") +
  geom_abline(intercept = 1, slope = 1, color="red") + theme_minimal()
```
...
tab_fun(table(realite_qda, resultat_qda), title="Matrice de confusion QDA") %>% add_header_above(c("","Prediction"=2))
tab_fun(round(
...
tab_fun(table(realite_qda_court, resultat_qda_court),
        title="Matrice de confusion QDA variables ressorties en ACM") %>% add_header_above(c("","Prediction"=2))
tab_fun(round(
...
tab_fun(round(head(tree$cp, -9),3), title="Complexité")
```
...
tab_fun(table(pred_tree, test[,32]), title="Matrice de confusion arbre de décision") %>% add_header_above(c("","Réalité"=2))
...
- La sensibilité est de `r round(table(pred_tree, test[,32])[1] / (table(pred_tree, test[,32])[1] + table(pred_tree, test[,32])[2])*100,3)`\%.
- La spécificité est de `r  round(table(pred_tree, test[,32])[4] / (table(pred_tree, test[,32])[3] + table(pred_tree, test[,32])[4])*100,3)`\%.
...
s_boosting<-predict(best_boost, type="prob", newdata=test)[,2]
s_rpart<-predict(treebis, newdata=test)[,2]
s_random<-predict(best_rf, type="prob", newdata=test)[,2]
s_bagging<-predict(best_bagging, type="prob", newdata=test)[,2]

s1_boosting<-prediction(s_boosting,test$moyenne_facteur)
s1_rpart<-prediction(s_rpart,test$moyenne_facteur)
s1_random<-prediction(s_random,test$moyenne_facteur)
s1_bagging<-prediction(s_bagging,test$moyenne_facteur)

roc_boosting<-performance(s1_boosting, measure="tpr", x.measure = "fpr")
roc_rpart<-performance(s1_rpart, measure="tpr", x.measure = "fpr")
roc_random<-performance(s1_random, measure="tpr", x.measure = "fpr")
roc_bagging<-performance(s1_bagging, measure="tpr", x.measure = "fpr")
plot(roc_rpart ,col="green", lwd=1)
abline(0,1,col="orange")
plot(roc_random,add=TRUE ,col="black", lwd=1)
plot(roc_bagging,add=TRUE ,col="blue", lwd=1)
plot(roc_boosting,add=TRUE ,col="red", lwd=1)
legend("bottomright",legend=c("rpart", "random", "bagging", "boosting"),
       col=c("green","black", "blue", "red"),lty=1,lwd=2)

#performance(s1_rpart,"auc")@y.values[[1]]
#performance(s1_random,"auc")@y.values[[1]]
#performance(s1_bagging,"auc")@y.values[[1]]
#performance(s1_boosting,"auc")@y.values[[1]]
```
...
erreurs <- matrix(c(
  ...
  mean(predict(best_tree, newdata = test[test$moyenne_facteur == "< 10", ], type = "class") !=
       ...
       test[test$moyenne_facteur == "> 10", 32]), performance(s1_rpart,"auc")@y.values[[1]],
  ...
  mean(predict(best_rf, newdata = test.data.complete[test.data.complete$moyenne_facteur == "> 10", ], type = "class") != test.data.complete[test.data.complete$moyenne_facteur == "> 10", 32]), performance(s1_random,"auc")@y.values[[1]],
  ...
  mean(predict(best_bagging, newdata = test.data.complete[test.data.complete$moyenne_facteur == "> 10", ]) != test.data.complete[test.data.complete$moyenne_facteur == "> 10", 32]),performance(s1_bagging,"auc")@y.values[[1]],
  ...
  mean(predict(best_boost, newdata = test.data.complete[test.data.complete$moyenne_facteur == "> 10", ]) != test.data.complete[test.data.complete$moyenne_facteur == "> 10", 32]),performance(s1_boosting,"auc")@y.values[[1]]
), byrow = TRUE, ncol = 4)
erreurs<-round(erreurs,3)
colnames(erreurs) <- c("Erreurs globales", "Erreurs < 10", "Erreur > 10", "AUC")
